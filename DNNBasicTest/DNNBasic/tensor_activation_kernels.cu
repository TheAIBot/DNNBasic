...
namespace dnnbasic
{
	...
	void tensorReLUDerivative(const tensor<T>& derivative_activation_function, const tensor<T>& affine_input, tensor<T>& output)
	{
		...
		{
			autoGraph::addKernelNode(ReLUKernelDerivative<T>, blockDim, gridDim, 0, derivative_activation_function.getGPUArrayConst(), affine_input.getGPUArrayConst(), output.getGPUArray());
		}
		else
		{
